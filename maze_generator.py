...
def check_neighbors(maze, x, y, width, height, checklist):
    directions = []
    if x > 0:
        if not maze.visited(2 * (x - 1) + 1, 2 * y + 1):       # 左边不是墙
            directions.append(Direction.LEFT)
    if y > 0:
        if not maze.visited(2 * x + 1, 2 * (y - 1) + 1):       # 上边不是墙
            directions.append(Direction.UP)
    if x < width - 1:
        if not maze.visited(2 * (x + 1) + 1, 2 * y + 1):       # 右边不是墙
            directions.append(Direction.RIGHT)
    if y < height - 1:
        if not maze.visited(2 * x + 1, 2 * (y + 1) + 1):       # 下边不是墙
...
def random_prime(map, width, height):
    start_x, start_y = (randint(0, width - 1), randint(0, height - 1))              # 随机起始点
...
def set_entrance_exit(maze):
    ...
    for i in range(maze.height):
        if maze.maze[i][1] == 0:        # 右边是通路
            maze.set_maze(0, i, 0)
            entrance = [0, i]
            break
    exit = []
    for i in range(maze.height - 1, 0, -1):
        if maze.maze[i][maze.width - 2] == 0:       # 左边是通路
